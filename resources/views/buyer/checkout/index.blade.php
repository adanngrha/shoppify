                ...
                <form action="{{ url('checkout/order') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div id="my-profile">
                        ...
                        <p>{{ $profile->phone_number }} <br>
                            {{ $address->address_name }}, {{ $address->city }}, {{ $address->province }},
                        ...
                        <table class="table table-borderless">
                            ...
                            <tbody>
                                ...
                                <tr class="text-center">
                                    <td width="50">
                                        <a href="detail-product/{{$cart->id}}" class="no-hover">
                                            <img src="{{ asset('img-product-upload/'.$cart->product_images[0]->picture) }}"
                                                width="80" alt="">
                                        </a>
                                    </td>
                                    <td class="align-middle text-left" width="400">{{ $cart->name }}</td>
                                ...
                                <tr>
                                    ...
                                    <td class="align-middle text-center">
                                        <select class="input-select" name="courier">
                                            ...
                                            <option name="courier" value="{{ $courier->id }}">{{ $courier->name }}
                                            </option>
                                            @endforeach
                                        </select>
                                        {{-- <select class="input-select">
                                        @if ($courier->id == 1)
                                            <option value="{{ $couriers->service()->id }}">{{ $couriers->service()->name }}
                                        </option>
                                        @elseif ($courier->id == 2)
                                        <option value="{{ $couriers->service()->name }}">
                                            {{ $couriers->service()->name }}</option>
                                        @else
                        ...
                        <div class="d-flex payment-method">
                            ...
                            {{-- <div class="d-flex flex-col ml-3">
                                <div class="input-radio m-2">
                                    <input type="radio" name="payment" id="payment{{$payment->id}}"
                                        value="{{$payment->id}}">
